...
```
Q1. 伪 P0 检测（客观规则 + 主观判断）：

    【第一步：客观规则预筛】
    对每个 P0 功能描述进行关键词检测，命中以下任意规则视为"疑似伪 P0"：
    
    □ 增强型词汇: 优化、增强、改进、美化、提升体验、更好
    □ 锦上添花词汇: 支持、兼容、适配、扩展、丰富
    □ 非核心动作: 推荐、个性化、智能匹配、自动填充
    □ 运营导向词汇: 活动、促销、营销、推广、引流
    □ 数据展示类: 统计、报表、看板、分析、可视化
    
    【豁免规则】以下情况直接判定为真 P0，跳过阻断性验证：
    ✓ 数据埋点: 用于验证需求成功指标的埋点需求
    ✓ 核心监控: 用于系统健康/告警的必要监控
    
    【第二步：阻断性验证】
    对"疑似伪 P0"（未命中豁免规则）进行人工判断：
    │   问："移除它后，核心业务闭环能否跑通？"
    ├─ [能跑通] → 确认伪 P0，每处扣 5 分 (上限 20 分)
    └─ [不能跑通] → 误判，保留为真 P0，不扣分
    
    【输出格式】
    | P0 功能 | 命中规则 | 豁免/阻断性验证 | 判定 |
    | 功能A | 增强型:优化 | 移除后不影响下单 | 伪P0 |
    | 功能B | 无命中 | - | 真P0 |
    | 埋点需求 | 数据展示类 | 豁免:验证成功指标 | 真P0 |
...
对每个 P0 功能，检查是否覆盖以下 5 类异常场景：

```
【异常路径检查清单】
□ 空状态: 列表为空、搜索无结果、首次使用无数据
□ 错误状态: 网络失败、接口报错、权限不足、参数非法
□ 边界状态: 超长文本、超大文件、并发冲突、重复提交
□ 中断状态: 支付中断、流程取消、超时未响应
□ 降级状态: 依赖服务不可用时的兜底方案
...
【输出格式】
| P0 功能 | 空状态 | 错误状态 | 边界状态 | 中断状态 | 降级状态 | 覆盖率 |
| 下单 | ✅购物车空提示 | ✅库存不足提示 | ❌ | ✅取消订单 | ❌ | 3/5 |
```

**C2. 逻辑死胡同 (扣 8分/处，上限 16分)**
流程有去无回（如弹窗无法关闭）、状态丢失、无退出路径。

**C3. 交互繁琐 (扣 5分)**
核心路径操作步骤 > 5 步，或单表单字段 > 10 个。
...
| 问题 | 证据提取 | 判定 | 扣分 |
| :--- | :--- | :--- | :--- |
| **Q1. 伪 P0 检测** | *{P0功能 + 命中规则 + 阻断性}* | {N}处伪P0 | **-{N×5}** |
