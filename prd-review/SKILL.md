...
*(基准 60 分加减，最低 0 分，最高 100 分)*

#### A. 规范性扣分 (Deductions)
* **[-20分] 无业务流程图:** *（参考脚本输出 `structure.has_flowcharts`）*
* **[-10分] 无目标用户:** 未定义适用人群/画像。
* **[-10分] 无数据埋点:** 未定义统计指标。
* **[-10分] 无异常流程:** 仅描述 Happy Path，未定义失败/超时/空状态。

#### B. 专业性加分 (Bonuses, 每项 +5分，上限 +30分)
* **字段定义:** 含类型/长度/校验规则表格。
* **权限与角色:** 区分多角色视图。
* **非功能需求 (NFR):** 含性能/安全/兼容性定义。
* **旧数据兼容:** 含迁移/洗数据策略。
* **版本记录:** 有清晰变更日志。
* **UI/UE 引用:** *（参考脚本输出 `structure.has_images`）* 含设计稿链接或图片。
...
| 检查项 | 证据/脚本数据 | 规则 | 变动分 |
| :--- | :--- | :--- | :--- |
| **业务流程图** | `has_flowcharts={bool}` | 无则扣分 | **-20 / 0** |
| **目标用户** | *{原文引用}* | 无则扣分 | **-10 / 0** |
| **数据埋点** | *{原文引用}* | 无则扣分 | **-10 / 0** |
| **异常流程** | *{原文引用}* | 无则扣分 | **-10 / 0** |
| **加分项** | *{亮点引用}* | 字段/权限/NFR/兼容/UI | **+{N}** |
...
### 📊 最终结论
* **逻辑分 (70%):** {L_Score} × 0.7 = {L_Weighted}
* **结构分 (30%):** {S_Score} × 0.3 = {S_Weighted}
* **最终得分：** **{Final_Score}**
* **评级：** **{S/A/B/C/D}**
* **结论：** **{✅ 批准 / ⚠️ 微修 / 🚧 重修 / ⛔️ 驳回}**
